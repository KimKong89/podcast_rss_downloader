...
def downloadEpisodesFromRSS(feed, fpath):
    ...
    print(f'''found {len(xml.xpath('//item'))} episode(s) ''')

    # loop over episodes
    for item in track(xml.xpath('//item'), 'downloading'):
...
def main():
    ...
    if not args.url:
        ...
        downloadEpisodesFromRSS(args.url, args.filepath)
        print('done')
